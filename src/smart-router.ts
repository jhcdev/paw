...
const SKILL_PATTERNS: [RegExp, string][] = [
  [/^(review|check)\s+(this|the|my)\s+(code|file|function|module|class)/i, "review"],
  [/(리뷰|검토|코드\s*리뷰|코드\s*검토)/, "review"],
  [/^(explain|what does|how does|what is)\b/i, "explain"],
  [/(설명|이해|뭐하는|어떻게\s*동작)/, "explain"],
  [/^(optimize|make.*faster|improve.*performance|speed up)/i, "optimize"],
  [/(최적화|성능\s*개선|빠르게)/, "optimize"],
  [/^(test|write tests?|add tests?|generate tests?)/i, "test"],
  [/(테스트\s*(작성|추가|생성|만들))/, "test"],
  [/^(document|add docs?|write docs?|jsdoc|tsdoc)/i, "document"],
  [/(문서화|문서\s*(작성|추가|생성))/, "document"],
  [/^(commit|git commit|commit message)/i, "commit"],
  [/(커밋\s*메시지|커밋\s*작성)/, "commit"],
  [/^(refactor)\s+(this|the|my)/i, "refactor"],
  [/(리팩토링|리팩터링)/, "refactor"],
];
